    ...
    <div class="bg-white border-b border-gray-200 px-6 py-4">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <FileText :size="28" class="text-blue-600" />
            日志详情
          </h1>
          <p class="text-sm text-gray-500 mt-1">共 {{ activeTab === 'actions' ? actionLogsTotal : activeTab === 'enterprise' ? enterpriseLogsTotal : total }} 条记录</p>
        </div>
      ...
      <div v-if="activeTab === 'sessions'" class="flex gap-4 mt-4">
        <input
          ...
          type="text"
          placeholder="搜索会话内容..."
          class="px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500 outline-none flex-1"
        />
        <input
        ...
        <select
          ...
          class="px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-200 focus:border-purple-500 outline-none text-sm"
        >
        ...
        <button
          ...
          class="px-4 py-2 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 disabled:opacity-50 flex items-center gap-2"
        >
      ...
      <div v-if="activeTab === 'actions'" class="flex gap-4 mt-4 flex-wrap">
        <select
          ...
          class="px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500 outline-none bg-white"
        >
          ...
          <div class="flex gap-2 bg-gray-100 p-1 rounded-lg">
            ...
            >
              <div class="flex items-center gap-2">
                <Building2 :size="16" />
                {{ tenant.company_name }}
                <span v-if="tenant.is_primary" class="px-1.5 py-0.5 text-xs bg-blue-100 text-blue-700 rounded">主</span>
              </div>
            </button>
          </div>
        </div>
        <div v-else-if="managedTenants.length === 1" class="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg border border-blue-200">
          <Building2 :size="18" class="text-blue-600" />
          <span class="text-sm font-medium text-blue-800">{{ managedTenants[0].company_name }}</span>
          <span v-if="managedTenants[0].is_primary" class="px-1.5 py-0.5 text-xs bg-blue-200 text-blue-900 rounded">主</span>
        </div>
        <select
      ...
      <div class="space-y-2">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span
                  ...
                  :class="{
                    'bg-green-100 text-green-700': log.level === 'INFO',
        ...
        <div v-if="actionLogsTotal > actionLogsPage * actionLogsPageSize" class="flex justify-center">
          <button
            ...
            class="px-6 py-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors flex items-center gap-2"
          >
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div v-if="enterpriseLogsTotal > enterpriseLogsPage * enterpriseLogsPageSize" class="px-4 py-3 border-t border-gray-200 flex items-center justify-center">
          <button
            ...
            class="px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50"
          >
            ...
            >
              ...
              <td class="px-3 py-2">
                <div class="text-xs text-gray-600">
                  <span class="text-green-600">{{ formatTokens(session.total_prompt_tokens) }}</span>
                  <span class="text-gray-400 mx-0.5">/</span>
                  <span class="text-blue-600">{{ formatTokens(session.total_completion_tokens) }}</span>
                  <span class="text-gray-400 mx-0.5">=</span>
              ...
              <td class="px-3 py-2">
                <button
                  class="px-2 py-0.5 text-xs text-blue-600 hover:bg-blue-50 rounded flex items-center gap-0.5"
    ...
    <div v-else-if="activeTab === 'statistics'" class="flex-1 overflow-auto p-6">
                ...
                <div class="flex gap-1 bg-gray-100 p-1 rounded-lg">
                  ...
                  >
                    <div class="flex items-center gap-1.5">
                      <Building2 :size="14" />
                      {{ tenant.company_name }}
                      <span v-if="tenant.is_primary" class="px-1 py-0.5 text-xs bg-blue-100 text-blue-700 rounded">主</span>
                    </div>
                  </button>
                </div>
              </div>
              <div v-else-if="isAdmin && managedTenants.length === 1" class="flex items-center gap-2 px-3 py-1.5 bg-blue-50 rounded-lg border border-blue-200">
                <Building2 :size="16" class="text-blue-600" />
                <span class="text-sm font-medium text-blue-800">{{ managedTenants[0].company_name }}</span>
                <span v-if="managedTenants[0].is_primary" class="px-1 py-0.5 text-xs bg-blue-200 text-blue-900 rounded">主</span>
              </div>
            </div>
      ...
      <template v-if="!isAdmin && userStats">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="p-4 bg-blue-50 rounded-lg">
              <div class="text-sm text-blue-600 font-medium">会话总数</div>
              <div class="text-2xl font-bold text-blue-700 mt-1">{{ userStats.chat_statistics?.total_sessions || 0 }}</div>
            </div>
            <div class="p-4 bg-green-50 rounded-lg">
              <div class="text-sm text-green-600 font-medium">消息总数</div>
              <div class="text-2xl font-bold text-green-700 mt-1">{{ userStats.chat_statistics?.total_messages || 0 }}</div>
            </div>
            <div class="p-4 bg-purple-50 rounded-lg">
              <div class="text-sm text-purple-600 font-medium">对话轮数</div>
              <div class="text-2xl font-bold text-purple-700 mt-1">{{ userStats.chat_statistics?.total_turns || 0 }}</div>
            </div>
            <div class="p-4 bg-orange-50 rounded-lg">
              <div class="text-sm text-orange-600 font-medium">Token 消耗</div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="p-4 bg-green-100 rounded-lg">
              ...
              <div class="text-xl font-bold text-green-800 mt-1">{{ formatTokens(userStats.chat_statistics?.total_prompt_tokens || 0) }}</div>
            </div>
            <div class="p-4 bg-blue-100 rounded-lg">
              <div class="text-sm text-blue-700 font-medium">Completion Tokens</div>
              <div class="text-xl font-bold text-blue-800 mt-1">{{ formatTokens(userStats.chat_statistics?.total_completion_tokens || 0) }}</div>
            </div>
            <div class="p-4 bg-gray-100 rounded-lg">
              <div class="text-sm text-gray-700 font-medium">总 Token</div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="p-4 bg-blue-50 rounded-lg">
              <div class="text-sm text-blue-600 font-medium">总操作数</div>
              <div class="text-2xl font-bold text-blue-700 mt-1">{{ userStats.action_statistics?.total_actions || 0 }}</div>
            </div>
            <div class="p-4 bg-green-50 rounded-lg">
            ...
            <div class="p-4 bg-purple-50 rounded-lg">
              ...
              <div class="text-2xl font-bold text-purple-700 mt-1">{{ userStats.action_statistics?.success_rate || 0 }}%</div>
            </div>
          </div>
        ...
        <div v-if="tenantStats" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="p-4 bg-purple-50 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-sm text-purple-600 font-medium">企业用户数</span>
              </div>
              <div class="text-2xl font-bold text-purple-700">{{ tenantStats.total_users }}</div>
            </div>
            <div class="p-4 bg-green-50 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                <MessageCircle :size="16" class="text-green-600" />
                <span class="text-sm text-green-600 font-medium">活跃用户数</span>
              </div>
              <div class="text-2xl font-bold text-green-700">{{ tenantStats.active_users }}</div>
            </div>
            <div class="p-4 bg-blue-50 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                <Hash :size="16" class="text-blue-600" />
                <span class="text-sm text-blue-600 font-medium">总会话数</span>
              </div>
              <div class="text-2xl font-bold text-blue-700">{{ tenantStats.total_sessions }}</div>
            </div>
            <div class="p-4 bg-orange-50 rounded-lg">
              <div class="flex items-center gap-2 mb-2">
                <BarChart3 :size="16" class="text-orange-600" />
                <span class="text-sm text-orange-600 font-medium">总Token消耗</span>
              </div>
          ...
          <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="p-3 bg-green-100 rounded-lg">
              <div class="text-sm text-green-700 font-medium">企业Prompt Tokens</div>
              <div class="text-lg font-bold text-green-800 mt-1">{{ formatTokens(tenantStats.total_prompt_tokens) }}</div>
            </div>
            <div class="p-3 bg-blue-100 rounded-lg">
              <div class="text-sm text-blue-700 font-medium">企业Completion Tokens</div>
              <div class="text-lg font-bold text-blue-800 mt-1">{{ formatTokens(tenantStats.total_completion_tokens) }}</div>
            </div>
            <div class="p-3 bg-gray-100 rounded-lg">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
              <List :size="20" class="text-blue-600" />
              操作日志详情
            </h3>
            <div class="text-sm text-gray-500">
              共 {{ actionLogsTotal }} 条记录
            </div>
          </div>
      ...
      <template v-else-if="isAdmin && tenantStats">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
              ...
              <div class="p-3 bg-purple-100 rounded-full">
                <Hash :size="24" class="text-purple-600" />
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
              <div>
                <div class="text-sm text-gray-500">总 Token 消耗</div>
                <div class="text-3xl font-bold text-gray-900 mt-1">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="p-4 bg-green-50 rounded-lg">
              <div class="text-sm text-green-600 font-medium">Prompt Tokens</div>
              <div class="text-2xl font-bold text-green-700 mt-1">{{ formatTokens(tenantStats.total_prompt_tokens) }}</div>
            </div>
            <div class="p-4 bg-blue-50 rounded-lg">
              <div class="text-sm text-blue-600 font-medium">Completion Tokens</div>
              <div class="text-2xl font-bold text-blue-700 mt-1">{{ formatTokens(tenantStats.total_completion_tokens) }}</div>
            </div>
            <div class="p-4 bg-gray-100 rounded-lg">
