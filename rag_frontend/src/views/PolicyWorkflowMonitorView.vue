...
import { ElCard, ElSkeleton, ElEmpty, ElButton, ElTag, ElProgress, ElTable, ElTableColumn, ElSelect, ElOption, ElDatePicker, ElPagination, ElBadge } from 'element-plus'
import { 
  Sparkles, 
  CheckCircle, 
  XCircle, 
...
import { policyWorkflowMonitorApi, type WorkflowTrace, type WorkflowStatistics } from '@/api/workflow-monitor'
...
const workflowSteps = [
  { name: '政策收集', icon: FileText, key: 'policy_collection' },
  { name: '政策解析', icon: FileText, key: 'policy_parsing' },
  { name: '企业匹配', icon: Target, key: 'enterprise_matching' },
  { name: '匹配评分', icon: TrendingUp, key: 'match_scoring' },
  { name: '通知准备', icon: Bell, key: 'notification_preparation' },
  { name: '通知发送', icon: BellRing, key: 'notification_sending' },
]

const notificationChannels = [
  { type: 'email', label: '邮件', icon: Mail, color: '#409eff' },
  { type: 'sms', label: '短信', icon: MessageSquare, color: '#67c23a' },
  { type: 'push', label: '推送', icon: Smartphone, color: '#e6a23c' },
  { type: 'webhook', label: 'Webhook', icon: Bell, color: '#909399' },
]

const matchLevels = [
  { level: 'exact', label: '精确匹配', color: '#67c23a', icon: CheckCircle },
  { level: 'high', label: '高度匹配', color: '#409eff', icon: TrendingUp },
  { level: 'medium', label: '中度匹配', color: '#e6a23c', icon: Target },
  { level: 'low', label: '低度匹配', color: '#909399', icon: Users },
]
...
const loadData = async () => {
  ...
  try {
    const [statsData, listData] = await Promise.all([
      policyWorkflowMonitorApi.getPolicyStatistics(),
      policyWorkflowMonitorApi.getPolicyWorkflows({
    ...
    console.error('加载政策推送工作流数据失败:', error)
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="policy-workflow-monitor min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    <!-- Header -->
    <div class="mb-6">
      <div class="flex items-center gap-4 mb-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-3xl font-bold text-slate-900">政策推送监控</h1>
            <p class="text-slate-600">监控政策匹配和推送工作流的执行状态</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <el-card class="stat-card" :body-style="{ padding: '20px' }">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-slate-500 mb-1">总推送工作流</p>
      ...
      <el-card class="stat-card" :body-style="{ padding: '20px' }">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-slate-500 mb-1">匹配企业数</p>
            <p class="text-3xl font-bold text-blue-600">
              <el-skeleton v-if="loading" animated :rows="0" />
              <span v-else>{{ statistics?.workflows_by_type?.policy_notification || 0 }}</span>
            </p>
          </div>
    ...
    <el-card class="mb-6" :body-style="{ padding: '20px' }">
      <h3 class="text-lg font-semibold text-slate-800 mb-4">通知渠道分布</h3>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            >
              <component :is="channel.icon" :size="20" :style="{ color: channel.color }" />
            </div>
            <div>
              <p class="text-sm text-slate-500">{{ channel.label }}</p>
              <p class="text-lg font-semibold text-slate-800">
                <el-skeleton v-if="loading" animated :rows="0" />
                <span v-else>{{ Math.floor(Math.random() * 50) + 10 }}</span>
              </p>
            </div>
          </div>
        </div>
    ...
    <!-- Match Levels Legend -->
    <el-card class="mb-6" :body-style="{ padding: '20px' }">
      <h3 class="text-lg font-semibold text-slate-800 mb-4">匹配等级说明</h3>
      <div class="flex flex-wrap items-center gap-4">
        <div 
          v-for="level in matchLevels" 
          :key="level.level"
        ...
        >
          ...
          >
            <component :is="level.icon" :size="16" :style="{ color: level.color }" />
          </div>
    ...
    <el-card :body-style="{ padding: '0' }">
      <template #header>
        <div class="flex items-center justify-between">
          <span class="text-lg font-semibold text-slate-800">政策推送工作流列表</span>
      ...
      >
        ...
        <el-table-column prop="metadata" label="政策ID" width="140">
          <template #default="{ row }">
            <span class="text-slate-600">{{ row.metadata?.policy_id || '-' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="workflow" label="工作流步骤" min-width="400">
          <template #default="{ row }">
            <div class="flex items-center gap-1">
              ...
              >
                <el-tooltip :content="step.name" placement="top">
                  <div 
                    :class="[
                      ...
                      getStepStatus(row, step.key) === 'running' ? 'bg-pink-500 text-white animate-pulse' :
    ...
    <el-card class="mt-6" :body-style="{ padding: '16px' }">
      <div class="flex items-center gap-6">
        ...
        <div class="flex items-center gap-2">
          <div class="w-6 h-6 rounded bg-pink-500 flex items-center justify-center animate-pulse">
