...
class LLMAdapterFactory:
    ...
    支持的提供商：zhipu, openai, claude, minimax, xinference, huggingface, modelscope, baichuan
    """
    ...
    def create_adapter(provider: Optional[str] = None) -> BaseLLMAdapter:
        ...
        # Claude
            ...
            logger.info(f"   - Base URL: {settings.CLAUDE_BASE_URL}")
        ...
        # MiniMax ⭐用户要求
            ...
            logger.info(f"   - 模型: {settings.MINIMAX_MODEL}")
            logger.info(f"   - Base URL: {settings.MINIMAX_BASE_URL}")
            logger.info(f"   - Group ID: {settings.MINIMAX_GROUP_ID or 'N/A'}")
        ...
        # Xinference（本地部署）
            ...
            logger.info(f"   - 模型: {settings.XINFERENCE_MODEL or 'default'}")
            logger.info(f"   - Base URL: {settings.XINFERENCE_BASE_URL}")
        ...
        # HuggingFace
            ...
            logger.info(f"   - 模型: {settings.HUGGINGFACE_MODEL}")
            logger.info(f"   - Base URL: {settings.HUGGINGFACE_BASE_URL}")
        ...
        # ModelScope（魔塔）
            ...
            logger.info(f"   - 模型: {settings.MODELSCOPE_MODEL}")
            logger.info(f"   - Base URL: {settings.MODELSCOPE_BASE_URL}")
        ...
        # BaiChuan（百川）
            ...
            logger.info(f"   - 模型: {settings.BAICHUAN_MODEL}")
            logger.info(f"   - Base URL: {settings.BAICHUAN_BASE_URL}")
        ...
        # 不支持的提供商
            ...
            raise ValueError(
                ...
                f"支持的提供商: zhipu, openai, claude, minimax, xinference, huggingface, modelscope, baichuan\n"
