...
class TestPersonalPageFeatures:
    ...
    async def test_change_invite_code(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            new_invite_code = response.json()["invite_code"]["code"]
...
if __name__ == "__main__":
    print("确保 FastAPI 服务器正在运行 (python -m uvicorn app.main:app --reload)")
    print("或者使用 docker-compose up -d")
    response = input("\n是否继续运行测试？(y/n): ")
    if response.lower() == 'y':
        asyncio.run(main())
    else:
